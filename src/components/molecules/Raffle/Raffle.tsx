...
import Modal from "@/components/modals/Raffle/RafflePutModal";
...
const Raffle = () => {
  ...
  return (
    <article className="text-white w-[80vw] bg-gray-800 p-4 rounded-md shadow-md">
          ...
          (raffleData) =>
            raffleData.status === "PENDING" && (
              ...
              >
                ...
                <p className="text-sm">Fecha: {raffleData.date}</p>
                <p className="text-sm">Hora: {raffleData.time}</p>
                <p className="text-sm">Estado: {raffleData.status}</p>
                <p className="text-sm">
                  Usuario Ganador: {raffleData.winnerUserId || "N/A"}
                </p>
                <p className="text-sm">
                  Combinación Ganadora:{" "}
                  {raffleData.winningCombinationId || "N/A"}
                </p>
                <button
      ...
      {showModal && (
        <Modal raffleId={selectedRaffleId || 0} onClose={handleCloseModal} />
      )}
    </article>
  );
};
